...
**📐 DESIGN.md IS LAW FOR FRONTEND WORK:** if a `DESIGN.md` exists in the project root, every frontend spawn (UI Designer, UX Designer, Frontend Engineer, Animations Expert, Design System Engineer) MUST receive the DESIGN.md file in its spawn prompt — it outranks their taste and any generic convention. All 5 frontend agents load the `impeccable` skill, but **impeccable is a command-driven tool, not passive craft** (23+ subcommands, each with its own `reference/<command>.md` playbook). Instruct every frontend spawn to run its protocol: ① `node <skill-base-dir>/scripts/context.mjs --target <path>` once (resolve `<skill-base-dir>` from the loader's base-dir hint) — this is what loads and enforces DESIGN.md, ② route the request to ONE subcommand from SKILL.md's Commands table, ③ follow that subcommand's playbook, ④ load `reference/craft-floor.md` before editing UI. Hand the agent the target path, not the file contents — the skill reads DESIGN.md itself.
